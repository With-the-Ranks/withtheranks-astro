...
export function Player({ lerp = THREE.MathUtils.lerp }) {
  ...
  useFrame((state) => {
    ...
    const velocityLength = Math.sqrt(velocity.x * velocity.x + velocity.y * velocity.y + velocity.z * velocity.z);
    ...
    // update axe
    axe.current.children[0].rotation.x = lerp(axe.current.children[0].rotation.x, Math.sin((velocityLength > 1) * state.clock.elapsedTime * 10) / 6, 0.1);
    ...
    if (jump && grounded) {
      ref.current.setLinvel({ x: direction.x, y: 7.5, z: direction.z }, true); 
    }
  })
  return (
    <>
